...
try {
    ...
    if ($r === 'exp_projects') {
        if ($method === 'POST') {
            ...
            ok(['id' => insert($pdo, 'experience_projects', pick($b, ['name', 'meta', 'logo', 'org', 'role', 'responsibility', 'sort_order']) + ['experience_id' => $eid])]);
        }
        if ($method === 'PUT') { update($pdo, 'experience_projects', $id, pick($b, ['name', 'meta', 'logo', 'org', 'role', 'responsibility', 'sort_order'])); ok(); }
